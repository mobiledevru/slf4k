...
class BasicMarker(override val name: String) : Marker {
    ...
    private val referenceListLock = NSRecursiveLock()
    ...
    override fun iterator(): Iterator<Marker> {
        referenceListLock.lock()
        val cloneIterator = listOf(*referenceList.toTypedArray()).iterator()
        referenceListLock.unlock()
    ...
    override fun remove(reference: Marker): Boolean {
        referenceListLock.lock()
        val result = referenceList.remove(reference)
        referenceListLock.unlock()
        return result
    }
